...
The adoption pin is `.governance/docs.json`. Run the checker from the immutable standard revision to validate changed documents. The protected executor profile adds the installed checker with the authenticated PR base. Deployment and successful enforcement require separate readback and canary evidence; see [documentation verification](docs/information/documentation-verification.md). Report actual validation and publication results.
